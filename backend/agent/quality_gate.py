...
async def check_semantic_quality_async(event: HookEvent) -> tuple[bool, str, str]:
    """
    #43: LLM 语义检查 — 评估输出质量 (切题性 + 工具结果一致性 + 幻觉检测)。

    注意: 这是异步函数，需要在异步上下文中调用。
    仅在 GUARDIAN_ENABLED=True 时自动启用 (复用 Guardian LLM 配置)。
    """
    import asyncio
    ...
    try:
        ...
        prompt = (
            "评估以下 AI Agent 回复的质量。检查:\n"
            "1. 切题性: 回复是否回答了用户的问题？\n"
            "2. 一致性: 回复是否与工具返回的结果一致？\n"
            "3. 幻觉: 回复中是否有工具结果中不存在的捏造信息？\n\n"
            f"工具结果:\n" + "\n".join(tool_summaries) + "\n\n"
            f"Agent 回复:\n{final_answer[:500]}\n\n"
            "如果质量合格，回复: PASS\n"
            "如果有问题，回复: FAIL|问题描述|修正建议"
        )

        resp = await asyncio.wait_for(
            llm.chat_completion(
                messages=[
                    {"role": "system", "content": "你是输出质量评估器。"},
        ...
        if len(parts) >= 3:
            return False, parts[1].strip(), parts[2].strip()
        return False, result[:200], "请检查回复的准确性"

    except Exception as e:
        logger.debug(f"Semantic quality check skipped: {e}")
        return True, "", ""
