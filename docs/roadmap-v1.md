...
**现状**: `run_command` 只有基础的沙箱路径限制。
**Codex 做法**: `execpolicy/` — .rules 文件定义白名单/黑名单 + 危险命令检测。

**服务端适配**:
- 不用文件 (.rules)，改为数据库/配置存储，租户管理员通过 API 配置
- 内置 dangerous 命令检测列表 (rm -rf, sudo, chmod 777 等)

**要做的事**:
- 新增 `core/exec_policy.py`:
  - `is_safe_command(cmd)` — 白名单判定
  - `is_dangerous_command(cmd)` — 危险命令检测 (参考 Codex banned_prefix 列表)
- 在 `security_hooks.py` 的 pre_tool_use hook 中对 `run_command` 做命令检查
- 租户级别的命令白名单配置 (通过 HookRule 机制)

**收益**: 防止 Agent 执行危险命令，5000 用户场景尤其重要。

### 2.2 Secrets 输出脱敏

**现状**: 没有 secret 脱敏机制。
**Codex 做法**: `redact_secrets()` 函数自动替换 secret 值为 `[REDACTED]`。

**服务端适配**:
- Secret 来源: 服务端 `.env` 中的 API Key 等，不是用户 keyring
- 脱敏范围: 工具输出 (防止 `run_command` 泄露 env) + LLM 输入 (防止 prompt injection)
- 租户级 secret pattern 配置 (管理员通过 API 设置)

**要做的事**:
- 新增 `core/secret_redactor.py`:
  - 启动时从 `.env` 和配置中收集 secret values
  - `redact(text)` — 替换匹配的 secret 值为 `[REDACTED_*]`
- 在 `post_tool_use` hook 中对工具输出做脱敏
- 在 `_build_assistant_message()` 前对 tool result content 做脱敏

**收益**: 防止 API Key、数据库密码等通过 Agent 输出泄露到前端。
...
| **第 6 批** | 2.1 命令安全 + 2.2 Secrets 脱敏 | 安全基础，无依赖 |
